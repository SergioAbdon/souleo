...
async function selecionarContexto(ctx){
  ...
  // Sincronizar dados do workspace para localStorage (clinicas)
  try{
    var ws = ctx.workspace;
    var cls = getClinicas();
    var existing = cls.find(function(c){ return c.id === ws.id; });
    var wsData = {
      id: ws.id, nome: ws.nomeClinica||'Consultório', slogan: ws.slogan||'',
      tel: ws.telefone||'', tel2: ws.telefone2||'',
      rua: ws.rua||'', num: ws.num||'', compl: ws.compl||'',
      bairro: ws.bairro||'', cidade: ws.cidade||'', uf: ws.uf||'', cep: ws.cep||'',
      end: ws.endereco||'', p1: ws.corPrimaria||'#1E3A5F', p2: ws.corSecundaria||'#2563EB',
      logoB64: ws.logoB64||ws.logoUrl||'', valor:0, padrao:true
    };
    if(existing) Object.assign(existing, wsData);
    else cls.push(wsData);
    setClinicas(cls);
    localStorage.setItem('leo_clinica_ativa_v21', ws.id);
  }catch(e){ console.warn('Sync workspace→localStorage:', e); }
