// ══════════════════════════════════════════════════════════════════
// LEO v7 · MOTOR V6 — Ecocardiograma Transtorácico
// Extraído intacto do laudo_leo_v6.html (linhas 3986-5100)
// Este é o motor de cálculo cardíaco original.
// Funciona acoplado ao HTML da sidebar (campos b7-b62).
//
...
//   - gerarConclusao(d)→ retorna array de conclusões
//   - renderizarLaudo(d) → renderiza no HTML (A4 preview)
//
// DEPENDÊNCIAS DO DOM:
//   - Inputs: b7-b62, peso, altura, sexo, ritmo, b27, wilkins-toggle
//   - Outputs: #achados-body, #conclusao-list, #params-tbody
//   - Helpers: escH() do 02-utils.js
//
// PARA ACOPLAR NO V7:
//   1. Incluir este arquivo via <script src="motores/motorv6.js">
...
function j8(d){
  ...
  const vdA=d.b13?d.b13>35:false;
  const adA=d.b25?(d.sexo==='F'?d.b25>27:d.b25>32):false; // AD em branco = normal (não é medido rotineiramente)

  // AD em branco = assume normal (não medido rotineiramente)
  const adNormal = !adA; // true se normal OU não medido

  const nomes = [];
  if(!aeA && (d.b8||d.b24)) nomes.push('Átrio esquerdo');
  if(!adA && d.b25) nomes.push('Átrio direito'); // só lista se foi medido
  if(!veA && d.b9) nomes.push('Ventrículo esquerdo');
  if(!vdA && d.b13) nomes.push('Ventrículo direito');

  // Contar câmaras efetivamente alteradas
  const totalAlteradas = [aeA,veA,vdA].filter(Boolean).length + (adA?1:0);

  // Nenhuma alterada → todas normais
  if(totalAlteradas === 0) return 'Câmaras cardíacas com dimensões normais.';

  // AD não medido conta como "normal implícito" para decidir "demais"
  const normaisVisiveis = nomes.length;
  const normaisTotal = normaisVisiveis + (!d.b25 && adNormal ? 1 : 0);

  if(normaisTotal === 0) return '';
  if(normaisVisiveis === 0 && normaisTotal > 0) return 'Demais câmaras cardíacas com dimensões normais.';
  if(normaisTotal === 1 && normaisVisiveis === 1) return nomes[0]+' com dimensões normais.';
...
if(typeof registrarMotor === 'function'){
  registrarMotor('eco_tt', {
    tipo: 'eco_tt',
    nome: 'Ecocardiograma Transtorácico (Motor V6)',
    calcular: calcAll,
...
console.log('%c🫀 Motor V6 (ECO TT) carregado — DICOM-ready ('+Object.keys(DICOM_TO_DOM).length+' campos) · i18n-prepared','color:#059669;font-weight:bold;font-size:11px;');
