...
| Critério | **A — cron renova, Direx suspende** | **B — renova até `pagoAte`** | **C — só alerta** |
|---|---|---|---|
| O que muda no dado | `franquiaUsada → 0`, `cicloFim += 30d` na virada; grava log | idem, mas só enquanto `cicloFim < pagoAte`; Direx grava `pagoAte` ao confirmar pagamento | nada muda |
| Quem precisa agir todo mês | ninguém (só para **cortar** um inadimplente) | o Direx, em toda conta, todo mês (marcar pago) | o Direx, em toda conta, todo mês (estender) |
| Risco de dar franquia de graça | **Alto se ninguém suspender.** Quem não pagar continua emitindo até alguém mudar o status. Detecta-se rápido (a lista de inadimplentes existe), mas o prejuízo é de laudos já emitidos. | **Baixo.** O sistema nunca renova sem alguém ter afirmado que o dinheiro entrou. | **Nenhum.** Ninguém emite de graça — o problema é o oposto: quem pagou também para. |
| Risco do lado do cliente | baixo — ninguém para no dia 31 | médio — se o Direx esquecer de marcar o pagamento, a conta para igual hoje | **alto — é o estado atual:** todo mundo para no dia 31 |
| Trabalho manual restante | cobrar (fora do sistema) + suspender inadimplente | cobrar + registrar pagamento + suspender | cobrar + estender conta por conta |
| Campos novos | `status` na assinatura (`'ativa' \| 'suspensa'`) | `status` + `pagoAte` + gravação no fluxo de pagamento do Direx | nenhum (o alerta de expiração já existe em `direx/painel/page.tsx:110-135`) |
| Esforço | **Pequeno** — 1 rota de cron ~60 linhas no molde do `cleanup-worklist`, 1 linha no `vercel.json`, 1 seletor no painel | **Médio** — cron + campo + UI de pagamento + regra de Firestore para `pagoAte` | **Muito pequeno** — o alerta já existe; falta o e-mail/aviso ao médico, X dias antes |
| Pré-requisito comum | corrigir o join `workspaceId` da §1(a) — senão nem cron nem painel acham as contas novas | mesmo | mesmo (o alerta também depende do join) |
...
- **O que faz:** `franquiaUsada: 0`, `cicloFim: cicloFim + 30d` (a partir do `cicloFim`, não de
  ...
  (mesmo padrão de `marina/route.ts:478-481`).
...
Independente de A/B/C: o join por `workspaceId` (§1a) precisa ser corrigido, senão as contas
