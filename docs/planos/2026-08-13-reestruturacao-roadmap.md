...
| 2 | Agenda + Seção 2 | Migração visual da Worklist pro padrão novo + execução do plano de correção já revisado pela tríade (`docs/planos/2026-08-12-plano-correcao-secao2-worklist.md` — 12 tarefas: recepção destravada, corridas CPF/ACC, import Feegow no servidor) | 1 | 2 | ✅ **CONCLUÍDO E NO AR** (regras 8a2d6730 + merge e2804a5 15/08) |
| 3 | Catálogo de tipos de laudo | `tiposLaudo/{id}` + seed (eco/carótidas/ecg/mapa/holter/ergométrico) + subseção Clínica→Tipos de laudo + fluxo laudo `texto` (TipTap c/ modelo) e `pdf` (anexo via rota server) + regra+teste | 1 | 3 | ✅ **CONCLUÍDO E NO AR** (regra 806dc762 + seed 3 ws + merge a84181a 15/08) |
| 4 | Pacientes | `/pacientes`: busca nome/CPF + ficha + linha do tempo dos exames (dados existentes) | 1 | 3–4 | ✅ **CONCLUÍDO E NO AR** (merge master be3cfc2 + fix 5b8ee60, 16/08; sem publicação de regra — deploy só de código) |
| 5 | Integrações | `/integracoes`: cartões Feegow/Orthanc/Wader com estado, testar conexão, credencial write-only, mapeamentos migrados do LocalModal | 1 | 4 | ✅ **CONCLUÍDO E NO AR** (regra bbc98256 + migração gravada em 2 locais + merge f2c9e78, 19/08). Falta a virada na clínica: atualizar o Wader e confirmar imagem entrando; limpeza dos campos antigos depois disso. ADR: `docs/decisoes/2026-08-19-sub-plano-5-integracoes.md` |
