...
- ~~**Regraduação após c≥3** (L143-149): a queda para Grau II é matematicamente equivalente ao Algoritmo B em **todos** os casos testados (com c≥3 entre 4 critérios, sempre há ≥2 dos 3 critérios de pressão de enchimento positivos). Nenhuma discrepância nesse trecho.~~
  > **ANULADA em 29/08/2026 (revisão final, achado F1).** A prova acima depende da premissa `c≥3`, que a **T2** desta onda derrubou: a entrada do ramo A passou a ser a MAIORIA dos avaliados, e com isso `c=2` (de 2 ou de 3 critérios) também abre a graduação — aí a Fig. 8 pode estar EMPATADA e o Grau II saía mesmo assim, invertendo contra o ramo B (mesmos dados: Grau II com FE 60, Indeterminada com FE 35). O F1 fecha isso: a zona média do ramo A agora **reconta os 3 critérios de pressão da Fig. 8** pela mesma função do ramo B (`maioriaPressaoEnchimento`) — maioria positiva → Grau II · empate ou <2 avaliados → Indeterminada. Pinos em `tests/unit/diastologia-graduacao-fluxo-mitral.test.mjs` (f–j).
