...
- [ ] `firebase deploy --only firestore:indexes` — ANTES da verificação em produção (build de índice é assíncrono; `reconciliarCancelados` usa índice novo e o primeiro uso pode cair em FAILED_PRECONDITION até o índice terminar de construir, mesmo com o `.catch` que evita 502 na importação).
